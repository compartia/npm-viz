...
export function joinStatsInfoWithGraph(
    ...
    _.each(stats.dev_stats, devStats => {
        ...
        _.each(devStats.node_stats, nodeStats => {
            ...
            const nodeName = nodeStats.node_name in graph.nodes ? nodeStats.node_name :
                nodeStats.node_name +
            ...
            const nodesByName = graph.nodes[nodeName];

            nodesByName.device = devStats.device;
...
export class MetanodeImpl implements Metanode {
    ...
    leaves(): string[] {
        let leaves = [];
        let queue = [<Node>this];
        let metagraph: graphlib.Graph<GroupNode | OpNode, Metaedge>; // Defined here due to a limitation of ES6->5 compilation.
        while (queue.length) {
 ...
 */
export function createGraph<N, E>(name: string, type: string | number, opt = {}):
    graphlib.Graph<N, E> {
