...
import {
    GraphType, GroupNode, Metaedge, MetaedgeImpl, Metanode, Node,
    NodeType, OpNode, ROOT_NAME, SeriesGroupingType, SeriesNode, SlimGraph,
    createGraph, createMetaedge, createMetanode, createSeriesNode, getHierarchicalPath, getSeriesNodeName
} from './graph';
...
function addEdges(h: Hierarchy, graph: SlimGraph,
    ...
    let getPath = (node: Node | null, path: string[]): number => {
        let i = 0;
        while (node) {
            path[i++] = node.name;
